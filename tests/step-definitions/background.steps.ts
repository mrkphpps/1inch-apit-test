 ...
 */
Given('the User Service is available at {string}', async function (this: CustomWorld, url: string) {
  // Store the User Service URL in the World object
  ...
  this.userRequest = await request.newContext({
    baseURL: url,
  });
});
 ...
 */
Given('the Order Service is available at {string}', async function (this: CustomWorld, url: string) {
  // Store the Order Service URL in the World object
  ...
  this.orderRequest = await request.newContext({
    baseURL: url,
  });
});
 ...
 */
Given('a user with id {string} exists', async function (this: CustomWorld, userId: string) {
  ...
  const response = await this.userRequest.get(`/users/${userId}`);
